...
class EletronicDocument(models.Model):
 ...
 SEM VALOR FISCAL'
        ...
        prod = {
            ...
            'vFrete': "%.02f" % abs(item.frete) if item.frete else '',
            'vSeg': "%.02f" % abs(item.seguro) if item.seguro else '',
            'vDesc': "%.02f" % abs(item.desconto) if item.desconto else '',
            'vOutro': "%.02f" % abs(item.outras_despesas)
 ...
 SEM VALOR FISCAL'
        ...
        total = {
            ...
            'vProd': "%.02f" % sum(self.document_line_ids.mapped(
                "valor_bruto")),
            'vFrete': "%.02f" % abs(self.valor_frete),
            'vSeg': "%.02f" % abs(self.valor_seguro),
            'vDesc': "%.02f" % abs(self.valor_desconto),
            'vII': "%.02f" % self.valor_ii,
            'vIPI': "%.02f" % self.valor_ipi,
            'vIPIDevol': '0.00',
            'vPIS': "%.02f" % self.pis_valor,
            'vCOFINS': "%.02f" % self.cofins_valor,
            'vOutro': "%.02f" % abs(self.valor_despesas),
